...
const InvoiceListPage = () => {
  const { useAllInvoices } = useGSTInvoices();
  ...
  const handleExcelDownload = () => {
    ...
    XLSX.writeFile(
      wb,
      `Business_Culture_Invoice-${new Date().toISOString().split("T")[0]}.xlsx`
    );
  };
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredInvoices.length === 0 ? (
                  ...
                  filteredInvoices.map((invoice, index) => {
                    const uniqueKey = invoice.invoiceID
                      ? `${invoice._id}-${index}`
                      : `key-${index}`;
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-500 font-semibold ">
                          <div className="border border-blue-500 text-center uppercase">
                            <Link to={`/gst-invoice/${invoice._id}`}>
                              {(invoice.clientInfo &&
                                invoice.clientInfo.split("\n")[0]) ||
                                "Client Name"}
                            </Link>
                          </div>
                        </td>
